...
export default function EditUavModal({
  ...
  const [purchaseDate, setPurchaseDate] = useState<Date | undefined>(new Date(data.purchaseDate))
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="name" name="name" defaultValue={data.name} required className="text-base" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="code" name="code" defaultValue={data.code} required className="text-base" />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="purchaseDate" className="text-base">Ngày mua</Label>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant={"outline"}
                        className={`w-full justify-start text-left font-normal text-base ${!purchaseDate && "text-muted-foreground"}`}
                      >
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="activity" className="text-base">Trạng thái hoạt động</Label>
                  <Switch
                    id="activity"
                    checked={isActive}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="note" name="note" defaultValue={data.note} className="text-base" />
                </div>
              </div>
            </CardContent>
          </Card>

          <Button type="submit" className="w-full text-base">Cập nhật</Button>
        </form>
      </DialogContent>
    </Dialog>
  )
